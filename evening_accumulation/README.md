...
也可以直接双击项目根目录下的 `tools/继续下载.cmd`。它会安全核对旧进程，以新股票池中序号最大的已下载股票为 checkpoint，并从其前5只开始扫描。终端用 `EXISTS` 标记已有数据、`DOWNLOADED` 标记本次新增，并显示全市场序号、百分比、速度、预计剩余时间和失败数；按 `Ctrl+C` 可以再次暂停。
...
当前正式版本标记为 **基准1-1：科创板暴涨前建仓策略（日频V3）**。冻结代码、模型、平台原始记录、逐年测试和本地对齐结果统一保存在 `baselines/baseline_1/`。后续版本必须另建目录并与基准1-1使用相同回测口径比较，不直接修改基准快照。
