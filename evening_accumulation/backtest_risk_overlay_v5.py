...
OUTPUT_FOLDER = os.environ.get("BACKTEST_OUTPUT", "risk_overlay_v5")
THRESHOLD = 0.689688
TOP_PER_DAY = 8
MAX_POSITIONS = 14
    ...
    for date in dates:
        ...
        if previous_market_date in risk_frame.index:
            ...
            if not np.isfinite(risk_probability):
                risk_probability = 0.0
        previous_equity = float(curve[-1]["equity"]) if curve else INITIAL_CASH
        high_water = max(high_water, previous_equity)
        portfolio_drawdown = previous_equity / high_water - 1.0
        caution = risk_metrics["caution_threshold"]
        threshold = risk_metrics["threshold"]
        extreme = risk_metrics["extreme_threshold"]
        risk_allow_new = True
        if risk_probability >= extreme:
            risk_streak += 1; recovery_stage = 0; last_defensive_cap = 0.20
            risk_cap = 0.20; risk_allow_new = False
        elif risk_probability >= threshold:
            risk_streak += 1; recovery_stage = 0
            if risk_streak >= 2:
                last_defensive_cap = 0.60; risk_cap = 0.60
            else:
                risk_cap = 1.00
            risk_allow_new = False
        elif risk_probability >= caution:
            risk_streak = 0; risk_cap = 1.00; risk_allow_new = False
        else:
            risk_streak = 0
            if recovery_stage < 3 and last_defensive_cap < 1.0:
                recovery_stage += 1
                recovery_caps = [last_defensive_cap, max(0.70, last_defensive_cap), 1.00]
                risk_cap = recovery_caps[recovery_stage - 1]
                if recovery_stage >= 3:
                    last_defensive_cap = 1.0
            else:
                risk_cap = 1.00
        if portfolio_drawdown <= -0.15:
            hard_level = 2; hard_low_days = 0
        elif portfolio_drawdown <= -0.10 and hard_level == 0:
            hard_level = 1; hard_low_days = 0
        elif hard_level > 0 and risk_probability < caution:
            hard_low_days += 1
            if hard_low_days >= 5:
                hard_level -= 1; hard_low_days = 0
                if hard_level == 1:
                    high_water = previous_equity / 0.90
                elif hard_level == 0:
                    high_water = previous_equity
        hard_cap = 0.20 if hard_level == 2 else (0.50 if hard_level == 1 else 1.00)
        risk_cap = min(risk_cap, hard_cap)
        if hard_level == 2:
            risk_allow_new = False
        ...
        if not candidates.empty and risk_allow_new:
        ...
        curve.append({"date": date, "equity": equity, "cash": cash,
                      ...
                      "risk_cap": risk_cap, "risk_allow_new": risk_allow_new,
                      "hard_level": hard_level, "portfolio_drawdown": portfolio_drawdown})
