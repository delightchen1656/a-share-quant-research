"""Realistic local daily backtest of the STAR event baseline30 strategy.
...
OUTPUT_FOLDER = os.environ.get("BACKTEST_OUTPUT", "live_realistic_daily")
THRESHOLD = 0.689688
TOP_PER_DAY = 8
MAX_POSITIONS = 14
...
TAKE_ALL = 0.34
MAX_HOLDING_DAYS = 30
HOLDING_PERIOD_MODE = os.environ.get("HOLDING_PERIOD_MODE", "calendar")
...
def run() -> tuple[dict, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        for symbol, pos in list(positions.items()):
            ...
            if mark <= trigger_entry * (1.0 - STOP_LOSS):
                ...
                if ((not ENFORCE_STAR_PARTIAL_MINIMUM)
                  ...
                  if HOLDING_PERIOD_MODE == "trading"
                  else ((date - pos["entry_date"]).days >= MAX_HOLDING_DAYS)):
    ...
    summary = {
        "mode": "realistic local daily execution",
